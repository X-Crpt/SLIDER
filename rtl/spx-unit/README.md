# spx-unit — Keccak + SPHINCS+ Coprocessor for CVA6 (RV64)

A 64-bit native CV-X-IF coprocessor combining the HORCRUX Keccak-f
permutation core (originally developed in a sibling `crheepto_total`
project, not part of this repo), the SPHINCS+ THASH/PRF unified pipeline
and the WOTS+ chain-lengths accelerator. Replaces the previous `keccak_ip`
4-op narrow ALU (xor3 / xandn / rotxor) with a self-contained hashing
engine driven by 20 custom instructions.
...
```
spx-unit/hw/
├── include/spx_unit_pkg.sv    # opcode enum + CoproInstr table
├── keccak/                    # HORCRUX Keccak-f core (verbatim)
├── spx_unit_register.sv       # 25×64-bit register file w/ XOR + bulk wb
├── chain_lengths.sv           # WOTS+ base_w + checksum
├── sphincs_ops.sv             # THASH/PRF FSM (re-uses Keccak)
├── spx_unit.sv                # Top compute (mux/wb/result)
├── spx_unit_xif_id.sv         # CV-X-IF instruction decoder
├── spx_unit_xif_ex.sv         # CV-X-IF execute (FSM + result registration)
├── spx_unit_xif_cid.sv        # Compressed-instr predecode (passthrough)
└── spx_unit_xif.sv            # CV-X-IF top wrapper
```
...
- Frontend:
	- `spx_unit_xif_id.sv`: decodes `{funct3, funct7}` into internal opcodes and
		requests only the source registers needed by each instruction.
	- `spx_unit_xif_ex.sv`: classifies ops into single-cycle vs multi-cycle,
		captures instruction context (`hartid/id/rd`), and emits registered
		CV-X-IF result responses.
	- `spx_unit_xif.sv`: top CV-X-IF wrapper that stalls issue while a multi-cycle
		operation is in flight (`issue_ready = issue_ready_id & ~ex_busy`).

- Backend compute:
	- `spx_unit_register.sv`: 25x64-bit state register file (one Keccak lane each),
...
- Backpressure behavior:
	- During multi-cycle ops, `spx_unit_xif` deasserts `issue_ready`, so CVA6
...
The coprocessor is selected by `CVA6Cfg.CoproType = COPRO_SPX_UNIT`. Two
configs ship with it enabled by default:

- `core/include/cv64a6_imafdc_sv39_config_pkg.sv`
- `core/include/cv64a6_imac_crypto_config_pkg.sv`

`corev_apu/src/ariane.sv` instantiates `spx_unit_xif` from the
`gen_COPRO_SPX_UNIT` branch. The legacy `keccak_xif` IP (`rtl/keccak_ip/`)
is kept as `COPRO_KECCAK` for backwards compatibility.
...
`spx_unit_xif` deasserts `issue_ready` while a multi-cycle op is in flight, so
...
RTL + filelist integration complete. SW tests live under `sw/simple/` and
`sw/pqc-complete/` (see `sw/README.md`).
