...
//     stall issue_ready (via the busy logic in spx_unit_xif), and pulse
//     result_valid when done_keccak_o / done_sphincs_o fires.

module spx_unit_xif_ex
  import spx_unit_pkg::*;
#(
  ...
  input  spx_unit_pkg::opcode_t   opcode_i,
  ...
  // Op classification
  // --------------------------------------------------------------------------
  function automatic logic is_keccak_op (spx_unit_pkg::opcode_t op);
    return (op == OP_KSTART) || (op == OP_KPERM);
  endfunction

  function automatic logic is_sphincs_op (spx_unit_pkg::opcode_t op);
  ...
  function automatic logic op_writes_rd (spx_unit_pkg::opcode_t op);
  ...
  spx_unit_pkg::opcode_t insn_to_hash;
