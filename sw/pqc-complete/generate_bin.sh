...
usage() {
  cat <<'EOF'
Usage:
  sw/pqc-complete/generate_bin.sh [options] [FOLDER_NAME]
...
Options:
  -v, --variant <original|optimized|all>  Variant selection (default: all)
  ...
  -o, --out <path>                        Output root (default: sw/pqc-complete/generated_bin)
      --clean                              Remove all .bin files under output root and exit
  -h, --help                              Show this help

Examples:
  sw/pqc-complete/generate_bin.sh
  sw/pqc-complete/generate_bin.sh --variant optimized
  sw/pqc-complete/generate_bin.sh --name SPHINCS-128f-robust
  sw/pqc-complete/generate_bin.sh SPHINCS-256f-simple
  sw/pqc-complete/generate_bin.sh --test keygen
  sw/pqc-complete/generate_bin.sh --variant original --test-sign-open
  sw/pqc-complete/generate_bin.sh --variant optimized --test keygen
  sw/pqc-complete/generate_bin.sh --variant optimized --test sign
  sw/pqc-complete/generate_bin.sh --variant optimized --test sign-open
  sw/pqc-complete/generate_bin.sh --clean
EOF
}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../cva6" && pwd)"

VARIANT="all"
FOLDER_NAME="all"
TEST_MODE="auto"
OUT_ROOT="$SCRIPT_DIR/generated_bin"
CLEAN_ONLY=0
...
if [[ "$VARIANT" != "all" && "$VARIANT" != "original" && "$VARIANT" != "optimized" ]]; then
...
compile_one() {
  ...
  local test_dir="../../../sw/pqc-complete/$rel_from_pqc"
  local test_name
  local src_base
  local thash_file
  local elf
...
if [[ $found_any -eq 0 ]]; then
  echo "No PQC run.sh scripts found under sw/pqc-complete." >&2
  exit 1
fi
