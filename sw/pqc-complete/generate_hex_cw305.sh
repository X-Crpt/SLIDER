...
usage() {
  cat <<'EOF'
Usage:
  sw/pqc-complete/generate_hex_cw305.sh [options] [FOLDER_NAME]
...
Options:
  ...
  -o, --out <path>                        Output root (default: sw/pqc-complete/generated_hex_cw305)
...
OUT_ROOT="$SCRIPT_DIR/generated_hex_cw305"
...
[[ "$VARIANT" =~ ^(all|original|optimized)$ ]] || { echo "Invalid variant: $VARIANT" >&2; exit 1; }
...
variants=()
if [[ "$VARIANT" = "all" ]]; then variants=(original optimized); else variants=("$VARIANT"); fi
...
if [[ $found_any -eq 0 ]]; then echo "No PQC run.sh scripts found under sw/pqc-complete." >&2; exit 1; fi
