#!/bin/bash
# =============================================================================
# sw/pqc-complete/run_spx_cw305.sh - Interactive picker for SLH-DSA SPHINCS+
# variants, built with CW305-style compilation flags.
#
# This is a thin wrapper around sw/pqc-complete/run_spx_batch_cw305.sh that
# asks for a single (mode/variant/phase) combination and runs just that one
# simulation.
#
# Usage (from anywhere):
#     bash   sw/pqc-complete/run_spx_cw305.sh        # spawns a child shell
#     source sw/pqc-complete/run_spx_cw305.sh        # runs in the current shell
# =============================================================================
...
mapfile -t SCRIPTS < <(find "$THIS_DIR/original/DS/SLH-DSA" "$THIS_DIR/optimized/DS/SLH-DSA" \
                            -mindepth 2 -maxdepth 2 -name 'run.sh' \
                            -printf '%h/run.sh\n' 2>/dev/null | sort)

if (( ${#SCRIPTS[@]} == 0 )); then
    _rt_die "no per-test run.sh scripts found under $THIS_DIR/{original,optimized}/DS/SLH-DSA/*"
    return 1 2>/dev/null || exit 1
fi
...
for i in "${!SCRIPTS[@]}"; do
    ...
    if [[ "$dir" == */original/* ]]; then mode="[ORIGINAL]"; else mode="[OPTIMIZED]"; fi
    printf "  [%2d] %-50s %s\n" "$i" "$variant" "$mode"
done
...
if [[ "$selected_dir" == */original/* ]]; then selected_mode="original"
