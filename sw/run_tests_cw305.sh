#!/bin/bash
# =============================================================================
# sw/run_tests_cw305.sh - Interactive picker across both the "simple" unit
# tests and the "pqc-complete" SLH-DSA variants, built with CW305-style
# compilation flags (rv64imac/lp64, -Os, cw305 runtime).
#
# Currently only the SLH-DSA SPHINCS+ variants under
#   pqc-complete/{original,optimized}/DS/SLH-DSA/<variant>/
...
#
# Usage (from anywhere, either way works):
#     bash   sw/run_tests_cw305.sh        # spawns a child shell
#     source sw/run_tests_cw305.sh        # runs in the current shell
# =============================================================================
...
BATCH="$THIS_DIR/pqc-complete/run_spx_batch_cw305.sh"
...
for i in "${!SCRIPTS[@]}"; do
    ...
    rel="${dir#$THIS_DIR/}"
    name=$(basename "$dir")
    if [[ "$rel" == pqc-complete/original/DS/SLH-DSA/* ]]; then
        tag="[CW305 ✓ original]"
    elif [[ "$rel" == pqc-complete/optimized/DS/SLH-DSA/* ]]; then
        tag="[CW305 ✓ optimized]"
    else
...
if [[ "$SELREL" == pqc-complete/original/DS/SLH-DSA/* || "$SELREL" == pqc-complete/optimized/DS/SLH-DSA/* ]]; then
    if [[ "$SELREL" == pqc-complete/original/* ]]; then mode="original"; else mode="optimized"; fi
    variant="$NAME"
