...
static void keccak_hw_full_permute(const uint64_t in[25], uint64_t out[25]) {
     ...
     *   12 x spx_unit_load2 covers lanes  0..23 (two lanes per insn)
     *    1 x spx_unit_load  covers lane  24    (HW saturates idx+1 to 24, so we
     *                                       use a single-lane LOAD for clarity)
     * Total: 13 issue cycles vs 25 with single-lane LOAD. */
    for (uint64_t i = 0; i < 12; i++) {
        spx_unit_load2(in[2 * i], in[2 * i + 1], 2 * i);
    }
    spx_unit_load(in[24], 24);
