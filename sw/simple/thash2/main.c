...
static int run_thash2_case(const thash_variant_t *v, uint32_t *sw_cycles, uint32_t *hw_cycles,
                            uint32_t *sw_instret, uint32_t *hw_instret, uint32_t seed) {
...
int main(void) {
    ...
    for (size_t i = 0; i < sizeof(k_variants) / sizeof(k_variants[0]); i++) {
        for (uint32_t tc = 0; tc < 2; tc++) {
            ...
            uint32_t i_sw = 0;
            uint32_t i_hw = 0;
            int err = run_thash2_case(&k_variants[i], &c_sw, &c_hw, &i_sw, &i_hw, tc);
